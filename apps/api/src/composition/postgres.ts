...
export interface PostgresApiOptions {
  ...
  /** Platform-scoped authorization for provisioning and lifecycle; platform roles are not modeled yet. */
...
export function composePostgresApi(options: PostgresApiOptions): { readonly app: FastifyInstance; close(): Promise<void> } {
  ...
  const app = buildApiServer({
    ...
    tenantStates,
    tenantCommands: createTenantCommands({ persistence: tenantPersistence, authorizer: options.platformAuthorizer, audit, clock }),
