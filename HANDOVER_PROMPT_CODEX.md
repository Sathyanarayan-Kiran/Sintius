...
- CI: GitHub Actions (`.github/workflows/ci.yml`, decision D13). Decisions D13–D16 were accepted on 2026-09-24; D13 and D16 are implemented, D14 (PostgreSQL consumer queue) and D15 (OpenTelemetry in `platform/observability`) are next.
- Baseline: `npm run check` passes typecheck plus **152 unit tests and 19 PostgreSQL integration tests (171 total)** across four PostgreSQL suite files run with `--test-concurrency=1`. Run it before editing and confirm.
