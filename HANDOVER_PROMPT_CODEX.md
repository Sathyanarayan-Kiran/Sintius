...
- A mapped requirement is not implemented. A planned test is not a passing test. Never claim 100% from mapping percentages. Implementation evidence is tracked in `docs/implementation/implementation-evidence.json` (11 of 1,335 at the last update). Claim a requirement only when every test mapped to it is passing; the generator and `npm run check:evidence` reject anything else.
...
- CI: GitHub Actions (`.github/workflows/ci.yml`, decision D13). Decisions D13–D16 were accepted on 2026-09-24; D13, D14 (PostgreSQL consumer queue: migration 006, `PostgresEventPublisher`, `PostgresDeliveryListener`, `createDeliveryWorker`, `PostgresOutboxStore({ queue: { kind: "delivery", consumer } })`) D15 (OpenTelemetry: `platform/observability` facade, `startTelemetry` at the process root, migration 007 `trace_context`, lint forbidding direct `@opentelemetry` imports elsewhere) and D16 are implemented.
- Baseline: `npm run check` passes typecheck plus **161 unit tests and 32 PostgreSQL integration tests (193 total)** across four PostgreSQL suite files run with `--test-concurrency=1`. Run it before editing and confirm.
