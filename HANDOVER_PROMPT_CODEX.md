...
- CI: GitHub Actions (`.github/workflows/ci.yml`, decision D13). Decisions D13–D16 were accepted on 2026-09-24; D13, D14 (PostgreSQL consumer queue: migration 006, `PostgresEventPublisher`, `PostgresDeliveryListener`, `createDeliveryWorker`, `PostgresOutboxStore({ queue: { kind: "delivery", consumer } })`) and D16 are implemented; D15 (OpenTelemetry in `platform/observability`) is next.
- Baseline: `npm run check` passes typecheck plus **153 unit tests and 24 PostgreSQL integration tests (177 total)** across four PostgreSQL suite files run with `--test-concurrency=1`. Run it before editing and confirm.
