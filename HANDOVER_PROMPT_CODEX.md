...
- A mapped requirement is not implemented. A planned test is not a passing test. Never claim 100% from mapping percentages. Implementation evidence is tracked in `docs/implementation/implementation-evidence.json` (3 of 1,335 at the last update). Claim a requirement only when every test mapped to it is passing; the generator and `npm run check:evidence` reject anything else.
...
- Baseline: `npm run check` passes typecheck plus **158 unit tests and 25 PostgreSQL integration tests (183 total)** across four PostgreSQL suite files run with `--test-concurrency=1`. Run it before editing and confirm.
