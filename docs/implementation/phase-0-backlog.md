...
- **Implementation status:** Implemented for Phase 0. `jose` verifies OIDC and workload JWTs against the provider's JWKS with rotation (D8); SAML is federated through the IdP. MFA, expiry, token age and durable revocation (`credential_revocation`) are enforced on every request, and each attempt yields a redacted security fact and metric. Platform operators have their own audience, MFA and reviewed roles (D11). Remaining: IdP product choice, a revocation feed, and the admin-web login flow.
...
- **Implementation status:** Implemented for Phase 0. The dedicated active-tenant command requires the non-default `foundation:proof:execute` permission. PostgreSQL evidence proves concurrent exact retries commit one tenant-owned proof record, audit event, outbox event and stored response, with correlation carried across record, audit and outbox. It also proves changed-payload conflict, tenant-B read isolation and that the same key in tenant B cannot replay tenant A's response; command-level rollback injection is covered. `tests/integration/phase0-release-gate.test.ts` composes the chain on PostgreSQL: real tenant provisioning and activation, RBAC read live from the tenant role tables (deny by default, then grant, then revocation on the next command), the idempotent proof, dispatch as the `sintius_dispatcher` role with one broker refusal and a retry, per-tenant stream order, and a deduplicated full redelivery through the PostgreSQL inbox. `apps/api/tests/postgres-http.test.ts` repeats the proof through the Fastify ingress: tenants are provisioned and activated over HTTP, the real authentication service (only signature verification is a test double) admits the caller, and one `X-Correlation-Id` reaches the proof record, the audit row and the published envelope; replay, payload conflict, missing key, body tenant and credential revocation are covered. Decision D15 completes the observability criterion: `apps/api/tests/postgres-http.test.ts` proves one OpenTelemetry trace runs from the HTTP server span through the idempotent command (with its audit event), the outbox hand-over, the delivery hand-over and the consumer, all carrying the ingress correlation ID, with replay recorded as a replayed outcome. CI (D13) captures TAP reports and requirement coverage as evidence. Platform-operator identity with the production platform authorizer (D11) and real signed-token verification (D8) are now in the HTTP proof as well.
...
- **Pipeline controls.** `tools/requirements/story-dod.ts` and `tools/requirements/test-strategy.ts` are pipeline controls, not checklists. `check-pipeline-controls.mjs` runs in the CI release gate, after the unit and PostgreSQL suites, against this run's TAP reports:
  ...
  - The golden-dataset class is proven by `docs/implementation/golden/money-proration-allocation.json` and `platform/money/tests/golden-dataset.test.ts`: it recomputes every proration and largest-remainder-allocation fixture from the real `platform/money` functions, checks output against the stored expected value and a recomputed content hash (catching a hand-edited expected value with a stale hash), checks the dataset's own provenance hash, and runs every fixture twice to prove determinism.
- **Specification-derived test status.** `docs/implementation/specification-test-status.json` is the durable, reviewed status overlay for TC IDs on specification-derived stories (`US-MSR-<section>-<seq>`, regenerated fresh every run and always seeded `not_run`). `tools/requirements/specification-test-status.ts` validates it the same way as implementation evidence: `passing` requires a real automated test titled with the exact ID, and (in CI) that it passed in this run. `MSR-080-3B6F4B3FE4` (OIDC/OAuth2) and `MSR-080-477F636C27` (MFA) are claimed this way, each by its own directly titled test; `MSR-080-A555453F8D` (SAML SSO) is not claimed, since SAML is federated through the identity provider and is not demonstrable in-repo.
- **Runnable preview.** `npm start` composes the API on PostgreSQL (`apps/api/src/main.ts`): a real, configured identity provider when `SINTIUS_TENANT_JWKS_URL`/`SINTIUS_PLATFORM_JWKS_URL` are set, or a clearly labelled, in-memory, local-development-only key set generated fresh on every boot otherwise (`apps/api/src/local-development/identity.ts`, never a production credential). `npm run demo:exit` (`apps/api/src/local-development/exit-demonstration.ts`) runs the Phase 0 exit scenario end to end against a migrated database: a platform operator provisions and activates tenant A and tenant B; a tenant-A user runs one command twice with one idempotency key; the script queries the database and prints exactly one record, one audit event and one outbox event for tenant A, and none of that command's evidence for tenant B. It is safe to re-run: every run uses freshly generated tenant IDs.
